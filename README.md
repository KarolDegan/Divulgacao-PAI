...
```
PyMuPDF (fitz) - para extração via PyMuPDF
pdfplumber    - para extração via pdfplumber
```
...
1. **Prepare os arquivos**:
   - Coloque os PDFs no diretório `./pdf/`
...
3. **Acompanhe o processo**:
   - JSONs intermediários são salvos em `./json/`
   - HTMLs finais são salvos em `./html/`
...
### Entrada (`./pdf/`)
Arquivos PDF a serem processados
- Padrão esperado: `[SIGLA]-*.pdf` ou já `[SIGLA]_[NUMERO].pdf`
- Serão automaticamente renomeados para o padrão standardizado

### Intermediário (`./json/`)
Arquivos JSON com dados estruturados
- Um arquivo JSON por PDF processado
- Serve como intermediário antes da geração de HTML

### Saída (`./html/`)
Páginas HTML geradas
- Um arquivo HTML por JSON
- Formatado com estilos da folha CSS
...
1. Edite `enums/NomesChaves.py`:
   ```python
   class NomesChaves(Enum):
       CAMPO_NOVO = "nome_do_campo"
   ```

2. Crie padrão em enum apropriada (ex: `PadroesTitulo.py`)

3. O sistema automaticamente extrairá o novo campo

### Modificar Estilos

Edite `css/estilo.css` para alterar aparência do HTML gerado

### Ajustar Padrões de Limpeza

Modifique `enums/PadroesLixo.py` para diferentes padrões a remover

---

## 📊 Exemplo de Fluxo

**Entrada**: `pai-2024.pdf`

**Passo 1 - Padronização**:
```
pai-2024.pdf → pai_1.pdf
```

**Passo 2 - Extração/Processamento**:
```
pai_1.pdf → (extração) → dicionário de 16 campos
```

**Passo 3 - Serialização**:
```
dicionário → json/pai_1.json
```

**Passo 4 - Geração HTML**:
```
json/pai_1.json → html/pai_1.html (página interativa)
```
...
Exemplo de dicionário gerado pelo `ControllerProcessamentoPDF`:

```json
{
  "campo_1": "Data: 25/06/2026",
  "campo_2": "Número do Processo: 2026/123456",
  "campo_3": "Nome do Projeto: Sistema de Divulgação",
  "campo_4": "Descrição: Automação de extração de dados",
  ...
  "campo_16": "Observações: Processado com sucesso"
}
```
...
Estrutura com seções, tabelas e estilos aplicados
...
**`padroes_nome_logico(nome_arquivo, tipo)`**:
- `tipo=1`: Retorna `SIGLA_NUMERO` (ex: `pai_1`)
- `tipo=2`: Retorna `SIGLA` + último `NUMERO` (ex: `pai1`)
- `tipo=3`: Retorna apenas `SIGLA` (ex: `pai`)
...
- Verifique formato: deve estar em `./pdf/`
- Verifique padrão de nome: `[SIGLA]-*.pdf` ou `[SIGLA]_[NUMERO].pdf`

### Extração retorna campos vazios
- Verifique padrões em `enums/`
- Verifique se PDF contém o texto esperado
- Tente com diferentes métodos (fitz vs pdfplumber)

### HTML mal formatado
- Verifique `css/estilo.css`
- Verifique tags em `enums/Tags.py`
- Revise `processamento/ConstrucaoHTML/ConstrutorHTML.py`
...
2. Faça alterações mantendo estrutura existente
3. Teste o pipeline completo
4. Solicite merge na `dev` para review
